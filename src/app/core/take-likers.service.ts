...
export class TakeLikesService {
    ...
    postTakeLike(take: any, user: string) {
        ...
        let url = `${this.baseUrl}/${take._id}/likers/${user}`;
        return this.http.post(url, {}, options)
                        .pipe(catchError(this.handleError))
                        .subscribe();
    }

    deleteTakeLike(take: any, user: string) { //no es username, es id

        take.likers = take.likers.filter(likerUsername => likerUsername !== user);
        let headers = new HttpHeaders();
        headers = headers.append('Content-Type', 'application/json');
        headers = headers.append('Authorization', this.authService.token);
        let options = {headers: headers};
